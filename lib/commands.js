    ...
    switch (command) {
        ...
        case '/load':
            ...
            if (!target) {
                ...
                try {
                    const searchPattern = target === 'all' ? '*' : target;
                    const filesToLoad = fg.sync(searchPattern, { dot: true, onlyFiles: true, deep: 1 });
                    ...
                    for (const filePath of filesToLoad) {
                        try {
                            ...
                            totalNewTokens += estimatedTokens;

                            const existingIdx = state.loadedFiles.findIndex(f => f.name === filePath);
                            if (existingIdx !== -1) {
                                state.loadedFiles[existingIdx].content = content;
                            } else {
                                state.loadedFiles.push({ name: filePath, content: content, isProtected: false });
                            }
                            loadedNames.push(filePath);
                        } catch (err) {
                            p.log.warn(`Skipped ${filePath}: ${err.message}`);
                        }
                    }

                    // Display the "shove" amount in the success message
                    const tokenMsg = totalNewTokens > 0 ? pc.dim(` (+ ${totalNewTokens.toLocaleString()} tokens)`) : '';
                    s.stop(`Loaded ${loadedNames.length} file(s).${tokenMsg}`);
                    
                    if (loadedNames.length > 0) {
                        state.history.push({
                            role: 'assistant',
                            content: `Acknowledged. I have loaded and can now see the following files in my context: ${loadedNames.join(', ')}`
                        });
                    }
                } catch (error) {
