...
export function buildSystemPrompt() {
    ...
    prompt += `TOOL USAGE DIRECTIVE: If you need to perform an action (create files, directories, run scripts), you MUST use the provided 'execute_shell' tool.\n`;
    prompt += `CRITICAL FILE WRITING DIRECTIVE: Local models struggle with JSON escaping. Therefore, when you need to write a file, DO NOT try to put the code inside the JSON tool arguments. Instead, write the full code normally in a markdown block (e.g., \`\`\`python ... \`\`\`) in your chat response. Then, call the 'write_file' tool providing ONLY the 'path'. The system will automatically extract your markdown block and save it to the file.\n\n`;
