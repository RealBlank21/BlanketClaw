...
export async function executeTool(toolCall) {
    if (toolCall.function.name === 'execute_shell') {
        ...
        logDebug('tool_exec', `Requested execute_shell`, command);
        ...
        if (!isApproved) {
            ...
            if (p.isCancel(action) || action === 'deny') {
                logDebug('tool_deny', `User denied command execution`, command);
        ...
        if (isApproved) {
            ...
            try {
                ...
                logDebug('tool_success', `Command succeeded: ${command}`, { stdout, stderr });
                return stdout || stderr || "Command executed successfully with no output.";
            } catch (error) {
                s.stop(`Execution Failed: ${command}`);
                logDebug('tool_fail', `Command failed: ${command}`, { stderr: error.stderr, message: error.message });
                return `Error: ${error.stderr || error.message}`;
            }
        }
    }
    logDebug('tool_unknown', `Unknown tool requested`, toolCall);
    return "Unknown tool.";
}